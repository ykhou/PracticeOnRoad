...
public class Solution3 {
     ...
     */
    static ArrayList<Point> min = new ArrayList<>();
    public static ArrayList<Point> winMazeGift (int[][] maze) {
        ...
        dfs(maze, x, y, new ArrayList<Point>(), vis);
    ...
    public static void dfs(int[][] maze, int x, int y, ArrayList<Point> cur, boolean[][] vis) {
        for (Point p :
                cur) {
            System.out.print(p.x + ":" + p.y + " ");
        }
        ...
        vis[x][y] = true;
        dfs(maze, x - 1, y, cur, vis);
        dfs(maze, x, y - 1, cur, vis);
        dfs(maze, x + 1, y, cur, vis);
        dfs(maze, x, y + 1, cur, vis);
        vis[x][y] = false;
        if (flag) {
